...
inner join (select * from inventory i inner join rental r on r.inventory_id = i.inventory_id) res on res.film_id = f.film_id
group by f.title
order by veces_alquilada DESC;

-- Error Code: 1060. Duplicate column name 'inventory_id'
